...
This branch teaches Azure infrastructure provisioning and application deployment using four independent GitHub Actions workflows.
...
- [API build/test/deploy](.github/workflows/deploy-api.yml), [UI build/deploy](.github/workflows/deploy-ui.yml), and [tutorials build/deploy](.github/workflows/deploy-tutorials.yml): test/validate each container independently on every push or PR; a merge to `main` (or a manual run) additionally builds, pushes, and deploys it to its Azure Web App — full CI/CD for each container, on its own schedule.
